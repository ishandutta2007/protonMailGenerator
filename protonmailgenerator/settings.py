...
class Settings:
    ...
    # locations
    IS_RUNNING = True
    log_location = localize_path("protonMailGenerator", "logs")
    database_location = localize_path("protonMailGenerator", "db")
    OS_ENV = (
        "windows" if platform == "win32" else "osx" if platform == "darwin" else "linux"
    )

    specific_chromedriver = "chromedriver_{}".format(OS_ENV)
    chromedriver_location = localize_path("protonMailGenerator", "assets", specific_chromedriver)
    if not chromedriver_location or not path_exists(chromedriver_location):
        chromedriver_location = localize_path("protonMailGenerator", "assets", "chromedriver")

    # minimum supported version of chromedriver
    chromedriver_min_version = 2.36

    platform_name = "protonmailgenerator"

    # set a logger cache outside the protonMailGenerator object to avoid
    ...
    WORKSPACE = {"name": "protonMailGenerator", "path": environmental_variables.get("HOME")}

    DATABASE_LOCATION = localize_path("protonMailGenerator", "db", "protonmailgenerator.db")
